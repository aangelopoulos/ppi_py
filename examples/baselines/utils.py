...
def make_plots(
    ...
    baseline_facecolor="#C75146",
    baseline_strokecolor="#AD2E24",
):
    # Make plot
    num_intervals = 5
    num_scatter = 3
    ...
    baseline_intervals = df[(df.n == n) & (df.method == "Baseline")].sample(
        n=num_intervals, replace=False
    )
    ...
    for i in range(num_intervals):
        ...
        baseline_interval = baseline_intervals.iloc[i]
        ...
        if i == 0:
            ...
            plot_interval(
                ...
                baseline_interval.upper,
                0.4 + yshift,
                lighten_color(baseline_facecolor, lighten_factor),
    ...
    baseline_widths = [
        df[(df.n == _n) & (df.method == "Baseline")].width.mean() for _n in ns
    ]
    classical_widths = [
    ...
    for _n in ns:
        ...
        baseline_width_list += df[
            (df.n == _n) & (df.method == "Baseline") & df.trial.isin(trials)
        ].width.to_list()
