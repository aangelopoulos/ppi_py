...
from .ppi_py import ppi_mean_pointestimate, ppi_ols_pointestimate, safe_expit, safe_log1pexp, rectified_p_value
from .utils import compute_cdf, compute_cdf_diff
...
def crossppi_mean_pointestimate(
    ...
    Args:
        ...
        Yhat_unlabeled (ndarray): Predictions corresponding to the unlabeled data. Columns contain predictions from different models.
...
def crossppi_mean_ci(
    ...
    crossppi_pointest = crossppi_mean_pointestimate(
        Y,
        Yhat,
        Yhat_unlabeled
    )
...
def crossppi_quantile_ci(
    ...
    indicators_Yhat_unlabeled_bstrap = (Yhat_unlabeled_bstrap_mean[:, None] <= grid[None, :]).astype(float)
    indicators_Y_bstrap = (Y_bstrap[:, None] <= grid[None, :]).astype(float)
    indicators_Yhat_bstrap = (Yhat_bstrap[:, None] <= grid[None, :]).astype(float)
...
def crossppi_ols_pointestimate(
    X,
    Y,
    Yhat,
    X_unlabeled,
...
def crossppi_ols_ci(
    ...
    """Computes the cross-prediction-powered confidence interval for the OLS coefficients.
    ...
    for i in range(N):
        ...
        grads_hat_unlabeled[i, :] = (
            X_unlabeled[i, :]
            * (np.dot(X_unlabeled[i, :], crossppi_pointest) - Yhat_unlabeled_bstrap[i])
        )
    ...
    def rectified_logistic_loss(_theta):
        return (
            ...
            * np.sum(
            ...
            - 1
            / n
            * np.sum(-Yhat * (X @ _theta) + safe_log1pexp(X @ _theta))
            + 1
            / n
            * np.sum(-Y * (X @ _theta) + safe_log1pexp(X @ _theta))
        )
