...
def resample_datapoints(
    data_truth, data_pred, data_pred_unlabeled, w, w_unlabeled
):
    """
    ...
    resampled_indices = np.random.choice(
        np.arange(0, n + N), size=n + N, replace=True
    )
...
def ptd_bootstrap(
    ...
    if tuning_method is None:
        ...
        cross_cov_calibration = np.atleast_2d(
            np.cov(
                np.concatenate(
                    (coeff_calibration_list.T, coeff_pred_calibration_list.T)
                )
            )[:d, d:]
        )
        cov_pred_calibration = np.atleast_2d(
            np.cov(coeff_pred_calibration_list.T)
        )
        cov_pred_unlabeled = np.atleast_2d(np.cov(coeff_pred_unlabeled_list.T))
        if tuning_method == "optimal":
            tuning_matrix = cross_cov_calibration @ np.linalg.inv(
                cov_pred_calibration + cov_pred_unlabeled
            )
        elif tuning_method == "optimal_diagonal":
            tuning_matrix = np.diag(
                np.diag(cross_cov_calibration)
                / (np.diag(cov_pred_calibration) + np.diag(cov_pred_unlabeled))
            )
    ...
    ptd_pointestimate = coeff_pred_unlabeled @ tuning_matrix.T + (
        coeff_calibration - coeff_pred_calibration @ tuning_matrix.T
    )

    # PTD confidence interval
    # compute B point estimates using the bootstrap coefficient estimates and tuning matrix
    pointestimates = coeff_pred_unlabeled_list @ tuning_matrix.T + (
        coeff_calibration_list - coeff_pred_calibration_list @ tuning_matrix.T
    )
